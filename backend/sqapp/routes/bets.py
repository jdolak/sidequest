...
def get_bet_accepted(group_id):
    return sql_response(
        sql_many(
            g.db_session,
            "SELECT bet_id, group_id, seller_id, username, max_quantity, side, odds, question, description, status FROM AVAILABLE_BETS ab, BOUGHT_BETS bb, SQ_USERS u WHERE ab.bet_id = bb.bet_id AND bb.buyer_id = :user_id AND ab.group_id = :group_id and u.user_id = ab.seller_id",
